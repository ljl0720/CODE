...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_INIT_CLK_DIV;//时钟分频采用普通模式 400KHz
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;//下降沿时钟采集
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;//禁用旁路时钟 在经过分频后输出至CLK
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;//禁用节能模式 一直输出CLK信号
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;//数据宽度 1/4/8
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;//不使用硬件控制流
  SDIO_Init(&SDIO_InitStructure);
  ...
  SDIO_CmdInitStructure.SDIO_Argument = 0x0;//设置参数为0
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;//设置命令为空闲
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;//不使用响应
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;//无等待状态
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;//使用CPSM结构
