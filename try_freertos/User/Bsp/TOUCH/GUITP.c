...
extern  SPI_InitTypeDef  SPI_InitStruct;  //SPI句柄
/*******************************************************************
 * @name       :void GUI_DrawPPoint(u16 x,u16 y,u16 color)
 * @date       :2018-08-09 
 ...
 * @retvalue   :None
********************************************************************/
void GUI_DrawPPoint(u16 x,u16 y,u16 color)
{
...
//			SendBuff[j] = color;	
	  ...
	  // MYDMA_USART_Transmit(&SPI1_Handler,(u8*)SendBuff,SENDBUFF_SIZE); //启动传输       
		    
			//  HAL_UART_Transmit_DMA(&UART1_Handler,SendBuff,SEND_BUF_SIZE);
			//使能串口1的DMA发送 //等待DMA传输完成，此时我们来做另外一些事，点灯
		    //实际应用中，传输数据期间，可以执行另外的任务
		    
//                while(__HAL_DMA_GET_FLAG(&DMA_Handle_Tx,DMA_FLAG_TCIF0_4)==RESET)//等待DMA2_Steam7传输完成
//                {
//                    __HAL_DMA_CLEAR_FLAG(&DMA_Handle_Tx,DMA_FLAG_TCIF0_4);//清除DMA2_Steam7传输完成标志
//                   HAL_SPI_DMAStop(&SPI1_Handler);
//					}
//         LCD_CS_CLR;
//				 LCD_CS_SET;
//         HAL_DMA_Start(SPI1_Handler.hdmatx, (u32)SendBuff, (uint32_t)&SPI1_Handler.Instance->DR, SENDBUFF_SIZE);//开启DMA传输
//         __HAL_DMA_SET_COUNTER(DMA_Handle_Tx,2*240);
//while(__HAL_DMA_GET_FLAG(DMA_FLAG_TCIF1_5)==RESET){
