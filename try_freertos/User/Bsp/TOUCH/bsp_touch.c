...
//	uint8_t temp[1];			 
//	//保存校正结果!		   							  
//	temp[0]=tp_dev.xfac;//*100000000;//保存x校正因素      
//    BSP_I2C_EE_WriteBuffer(temp,SAVE_ADDR_BASE,4);   
//	temp[0]=tp_dev.yfac;//*100000000;//保存y校正因素    
//    BSP_I2C_EE_WriteBuffer(temp,SAVE_ADDR_BASE+4,4);
//	//保存x偏移量
//    BSP_I2C_EE_WriteBuffer((void*)(tp_dev.xoff),SAVE_ADDR_BASE+8,2);		    
//	//保存y偏移量
//	BSP_I2C_EE_WriteBuffer((void*)(tp_dev.yoff),SAVE_ADDR_BASE+10,2);	
//	//保存触屏类型
//	BSP_I2C_EE_WriteByte((void*)(tp_dev.touchtype),SAVE_ADDR_BASE+12);	
////	temp[0]=0X0A;//BSP_I2C_EE_WriteBuffer
//	BSP_I2C_EE_WriteByte(temp+13,SAVE_ADDR_BASE+13); 
}
...
{					  
//	u8 tempfac[1];
//	BSP_I2C_EE_ReadBuffer(tempfac,SAVE_ADDR_BASE+13,1);//读取标记字,看是否校准过！ 		 
//	if(tempfac[0]==0X0A)//触摸屏已经校准过了			   
//	{    												 
//		BSP_I2C_EE_ReadBuffer(tempfac,SAVE_ADDR_BASE,4);		   
//		tp_dev.xfac=(float)tempfac[0];///100000000;//得到x校准参数
//		BSP_I2C_EE_ReadBuffer(tempfac,SAVE_ADDR_BASE+4,4);		          
//		tp_dev.yfac=(float)tempfac[0];///100000000;//得到y校准参数
//	    //得到x偏移量
//		BSP_I2C_EE_ReadBuffer((void*)(tp_dev.xoff),SAVE_ADDR_BASE+8,2);			   	  
// 	    //得到y偏移量
//		BSP_I2C_EE_ReadBuffer((void*)(tp_dev.yoff),SAVE_ADDR_BASE+10,2);				 	  
// 		BSP_I2C_EE_ReadBuffer((void*)(tp_dev.touchtype),SAVE_ADDR_BASE+12,1);//读取触屏类型标记
		...
		{	
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					fac=(float)d1/d2;
					if(fac<0||fac>10)//不合格fac<0.95||fac>1.05
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
...
{			    		   
	...
	//所以上拉之前,必须使能时钟.才能实现真正的上拉输出
		...
		RCC_AHB1PeriphClockCmd(BSP_TOUCH_GPIOI_CLK|BSP_TOUCH_GPIOH_CLK|BSP_TOUCH_GPIOG_CLK,ENABLE);			//开启GPIOH时钟	//开启GPIOI时钟//开启GPIOG时钟
        

		//PH6
		GPIO_Initure.GPIO_Pin=BSP_TOUCH_SCK_PIN;            //PH10
	  ...
	  GPIO_Initure.GPIO_OType=GPIO_OType_PP;
		...
		GPIO_Init(BSP_TOUCH_GPIOH,&GPIO_Initure);     //初始化
        
		//PI3,8
		GPIO_Initure.GPIO_Pin=BSP_TOUCH_MOSI_PIN|BSP_TOUCH_CS_PIN; //PI3,8
		GPIO_Init(BSP_TOUCH_GPIOI,&GPIO_Initure);     //初始化
        
		//PH11
		GPIO_Initure.GPIO_Pin=BSP_TOUCH_PEN_PIN;            //PH11
		GPIO_Initure.GPIO_Mode=GPIO_Mode_IN;      //输入
		GPIO_Init(BSP_TOUCH_GPIOH,&GPIO_Initure);     //初始化
        
		//PG3
		GPIO_Initure.GPIO_Pin=BSP_TOUCH_MISO_PIN;            //PG3
		GPIO_Init(BSP_TOUCH_GPIOG,&GPIO_Initure);     //初始化
